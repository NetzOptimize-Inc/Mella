/*
  ...
  -----------------------------------------------------------------
  This sketch is a drop-in replacement template to use Preferences
  (NVS) instead of EEPROM for storing SSID/password and a persistent
  AP identifier. It also includes:
   - AP (provisioning) and STA (connect) logic
   - HTTP endpoints for /status, /scan, /config, /connect-status
   - factoryReset() via a push-button (long-press)
   - persistent AP id storage so "AmazonBox-XXXX" stays the same

  IMPORTANT: This is a template. If you want me to update your
  existing AmazonBox_RGB.ino file directly, either paste it here
  or re-upload and I'll perform an exact automated patch.

  How to use:
   - Upload this sketch to your ESP32 (or copy functions into your
     existing sketch replacing EEPROM code)
   - Wire a push-button between RESET_BUTTON_PIN and GND
   - Open Serial Monitor at 115200 baud

  Notes:
   - No EEPROM usage anywhere in this file.
