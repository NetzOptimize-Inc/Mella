...
class AlarmPingSender implements MqttPingSender {
    ...
    @Override
    public void init(ClientComms com) {
        this.com = com;
    ...
    @Override
    public void start() {
        String action = MqttServiceConstants.PING_SENDER + com.getClient().getClientId();
        Log.d(TAG, "Register alarm receiver to MqttService" + action);

        if (android.os.Build.VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU) {
            service.registerReceiver(alarmReceiver, new IntentFilter(action), Context.RECEIVER_VISIBLE_TO_INSTANT_APPS);
        } else {
        ...
        schedule(com.getKeepAlive());
        hasStarted = true;
    }

    @Override
    public void stop() {
        Log.d(TAG, "Unregister alarm receiver to MqttService" + com.getClient().getClientId());
        if (hasStarted) {
    ...
    public void schedule(long delayInMilliseconds) {
        ...
        try {
            ...
            // Add your custom error handling code here, such as requesting permissions or displaying an error message to the user
        }
    }

    class AlarmReceiver extends BroadcastReceiver {
        private WakeLock wakelock;
        private final String wakeLockTag = MqttServiceConstants.PING_WAKELOCK + that.com.getClient().getClientId();
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            IMqttToken token = com.checkForActivity(new IMqttActionListener() {
